...
options {
	...
	commaatom: fos | EXP;
	commafos: commaatom (COMMA (commaatom))* ; 
	
	rule: (commafos)? ((res)?  | ((IS fos)|(IMP (commit))) (REQ commafos )?  ) SEMI; 
	...
	commit :
 	(fos (COMMA fos (COMMA (fos | EXP) (COMMA fos)? )? )? ) 
 	(res)?;
 	
 	res :APOS fos COMMA fos (COMMA fos)?;
 	
 	exp : HASH VAL (PLUSMIN VAL)* VAL (PLUSMIN VAL)* ;
...
EXP options {
paraphrase = "an expression";
}: '#' ('+'|'-' )? ('0'..'9'|'?'|'a'..'z'|'A'..'Z'|'_'|'$'|WS|ML_COMMENT)+  (('+'|'-') ('0'..'9'|'?'|'a'..'z'|'A'..'Z'|'_'|'$'|WS|ML_COMMENT)+ )* 
('='|('>' ('=')?) |('<' ( '=')?) )
( '+'|'-')? ('0'..'9'|'?'|'a'..'z'|'A'..'Z'|'_'|'$'|WS|ML_COMMENT)+ (('+'|'-') ('0'..'9'|'?'|'a'..'z'|'A'..'Z'|'_'|'$'|WS|ML_COMMENT)+)*
;
