...
/** The CDC peer to peer task reads an incomming message from a socket and adds
 * the message to the peer to peer message transport service. 
 * 
 * @author Conor Muldoon
 *
 */
public class CDCP2PTask implements Runnable{
	StreamConnection socket;
	CDCPeerToPeerMTS p2pMTS;
	String app;
	Platform p;
	
	/** Creates an instance of the CDC peer to peer task.
	 * 
	 * @param s the incomming socket connection.
	 * @param mts the peer to peer message tranport service of the platform.
	 * @param applic the name of the agent platform.
	 * @param plat the local agent platform.
	 */
	CDCP2PTask(StreamConnection s,CDCPeerToPeerMTS mts,String applic,Platform plat){
