...
/** The peer to peer message transport service facilitates communication between agents in AFME in a peer
 * to peer manner.
 ...
 */
public class PeerToPeerMTS extends Service implements Runnable {
	 ...
	 */
	public PeerToPeerMTS(String[] args, Object[] agtNms,
		...
		try{
			...
			localAddress="socket://"+ssc.getLocalAddress()+':'+ssc.getLocalPort();
			...
			if(args.length>4){
				...
				dos.writeByte(DiscServer.REG);
				dos.writeUTF(app);
				dos.writeUTF(ssc.getLocalAddress());
				dos.writeInt(ssc.getLocalPort());
	...
	public void run() {
		...
		try{
			while(active){
				...
				b.insert(new P2PTask(socket,this,app,p));				
			}
