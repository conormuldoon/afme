...
public class ServerApp {
    ...
    public ServerApp() {
        ...
        try{
            ...
            MigrationStore store=new MigrationStore(String.valueOf(midpPort));
        ...
        if(args.length==0)new ServerApp();
        	...
        	try{
        		int port=Integer.parseInt(args[0]);
        		 MigrationStore store=new MigrationStore(args[0]);
                 MigrationServer migServer=new MigrationServer(port,new DesignFile(),4,store);
                 migServer.start();
        	}catch(NumberFormatException e){
        		
        		System.err.println("Enter an integer for the port number.");
        	}
        }
    }
}
